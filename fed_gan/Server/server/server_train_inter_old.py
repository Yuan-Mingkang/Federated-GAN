    ...
    for epoch in range(int(opt.epoch)):
        ...
        # for j in range(client_num):
            ...
            for i in tqdm(range(length//batch_size)):
                ...
                fake_temp = f'../data/dataset/fake_temp'
                shutil.rmtree(fake_temp)
                os.mkdir(fake_temp)
                for h in range(batch_size):
                    # fake_name = ''.join(B_name[h].split('.')[:-1]) + '.jpg'
                    fake_name = ''.join(B_name[h].split('.')[:-1]) + '.png'
                    fake_path = f'../data/dataset/fake/{fake_name}'
                    fake_temp_path = f'../data/dataset/fake_temp/{fake_name}'
                    utils.save_image(fake_B[h], fake_path, normalize=True)
                    tensor = fake_B[h]
                    fake_tensor = (tensor + 1) / 2
                    fake_tensor *= 255
                    fake_img = Image.fromarray(
                    fake_tensor.squeeze(0).permute(1, 2, 0).cpu().detach().numpy().astype('uint8')).convert('RGB')
                    fake_img.save(fake_temp_path)

                zip_fake_temp = '/'.join(fake_temp.split('/')[:-1])
                zip_fake_temp = os.path.join(zip_fake_temp, f'temp_zip.zip')
                zip_dir(fake_temp, zip_fake_temp)
                fr = open(zip_fake_temp, 'rb')
                temp_mask = fr.read()
                fr.close()
                # ----trans-jpg----
                # temp_fake_B = fake_B.cpu()
