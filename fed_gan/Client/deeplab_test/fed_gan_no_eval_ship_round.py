...
def main():
    ...
    with open(r'./fed_gan_no/test_cls0_result_round.txt', 'a+', encoding='utf-8') as cls0:
        cls0.truncate(0)
    with open(r'./fed_gan_no/test_cls1_result_round.txt', 'a+', encoding='utf-8') as cls1:
        cls1.truncate(0)
    with open(r'./fed_gan_no/test_miou_result_round.txt', 'a+', encoding='utf-8') as miou:
        miou.truncate(0)
    with open(r'./fed_gan_no/test_pa_result_round.txt', 'a+', encoding='utf-8') as pa:
        pa.truncate(0)
    with open(r'./fed_gan_no/test_dice_result_round.txt', 'a+', encoding='utf-8') as dice:
        dice.truncate(0)
    epochs = 1
    for epoch in range(epochs, 1001):
        with torch.no_grad():
            ...
            opts.ckpt = f"/home/poac/7TB/yuanmingkang/deeplab_WHU2/deeplab_WHU2_train/fed_gan_no/checkpoints/{epoch}_deeplabv3plus_resnet101_ship_os16.pth"
            ...
            for label_path,img_path,label_name1 in tqdm(zip(labels_files,images_files,labels_names)):
                ...
                pred = decode_fn(pred)
                pred = np.uint8(pred)
                Image.fromarray(pred).save(f'/home/poac/7TB/yuanmingkang/deeplab_WHU2/deeplab_WHU2_test/fed_gan_no/seg_data/{label_name1}')
        ...
        for i in range(0,opts.num_classes):
            ...
            with open(f'./fed_gan_no/test_cls{i}_result_round.txt', "a") as log_file:
                log_file.write('%s\n' % message)
        ...
        with open('./fed_gan_no/test_miou_result_round.txt', "a") as log_file:
            log_file.write('%s\n' % message)

        message = f"epoch:{epoch}, pa:{pa/len(images_files)}"
        with open('./fed_gan_no/test_pa_result_round.txt', "a") as log_file:
            log_file.write('%s\n' % message)

        message = f"epoch:{epoch}, dice:{dice/len(images_files)}"
        with open('./fed_gan_no/test_dice_result_round.txt', "a") as log_file:
            log_file.write('%s\n' % message)
