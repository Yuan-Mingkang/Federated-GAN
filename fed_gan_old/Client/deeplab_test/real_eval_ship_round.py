...
def main():
    ...
    opts.save_val_results_to = "/home/poac/4TB/yuanmingkang/deeplab/deeplab_city/deeplab_ship_test/real/result"
    ...
    with open(r'./real/test_cls0_result_round.txt', 'a+', encoding='utf-8') as cls0:
        cls0.truncate(0)
    with open(r'./real/test_cls1_result_round.txt', 'a+', encoding='utf-8') as cls1:
        cls1.truncate(0)
    with open(r'./real/test_miou_result_round.txt', 'a+', encoding='utf-8') as miou:
        miou.truncate(0)
    with open(r'./real/test_pa_result_round.txt', 'a+', encoding='utf-8') as pa:
        pa.truncate(0)
    with open(r'./real/test_dice_result_round.txt', 'a+', encoding='utf-8') as dice:
        dice.truncate(0)
    epochs = 1
    for epoch  in range(epochs, 1001):
        with torch.no_grad():
            ...
            opts.ckpt = f"/home/poac/7TB/yuanmingkang/deeplab_WHU2/deeplab_WHU2_train/real/checkpoints/{epoch}_deeplabv3plus_resnet101_ship_os16.pth"
            ...
            for label_path,img_path,label_name1 in zip(labels_files,images_files,labels_names):
                i = i + 1
                ...
                Image.fromarray(pred_save).save(f'/home/poac/7TB/yuanmingkang/deeplab_WHU2/deeplab_WHU2_test/real/seg_data/{label_name1}')
        ...
        # with open('./real/test_cls_result_round.txt', "a") as log_file:
        #     log_file.write('%s\n' % message)
        for i in range(0,opts.num_classes):
            ...
            with open(f'./real/test_cls{i}_result_round.txt', "a") as log_file:
                log_file.write('%s\n' % message)
        ...
        with open('./real/test_miou_result_round.txt', "a") as log_file:
            log_file.write('%s\n' % message)

        message = f"epoch:{epoch}, pa:{pa/len(images_files)}"
        with open('./real/test_pa_result_round.txt', "a") as log_file:
            log_file.write('%s\n' % message)

        message = f"epoch:{epoch}, dice:{dice/len(images_files)}"
        with open('./real/test_dice_result_round.txt', "a") as log_file:
            log_file.write('%s\n' % message)
