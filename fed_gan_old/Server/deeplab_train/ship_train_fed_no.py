...
def get_argparser():
    ...
    parser.add_argument("--mask_path", type=str, default='/home/poac/4TB/yuanmingkang/dataset/WHU_Building/Satellite dataset вё (global cities)/train/mask_aug',
                        help="path to mask")
    parser.add_argument("--train_path", type=str, default='/home/poac/7TB/yuanmingkang/fed_gan_WHU2/data/dataset/fake_B_no',
                        help="path to fed_gan_fake")
...
def main():
    ...
    epoch_loss = 0
    a = 0
    with open(r'./fed_gan_no/loss/loss_deeplab.txt', 'a+', encoding='utf-8') as loss_deeplab:
        loss_deeplab.truncate(0)
    with open(r'./fed_gan_no/loss/epoch_loss_deeplab.txt', 'a+', encoding='utf-8') as epoch_loss_deeplab:
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for images, labels, names in tqdm(train_loader):
            ...
            for h in range(opts.batch_size):
                ...
                Image.fromarray(pred_train).save(f'/home/poac/7TB/yuanmingkang/deeplab_WHU2/deeplab_WHU2_train/fed_gan_no/seg_data/{names[cur_itrs*opts.batch_size+h][1]}')
            ...
            with open('./fed_gan_no/loss/loss_deeplab.txt', "a") as log_file:
                log_file.write('%s\n' % message)
        ...
        save_ckpt(f'./fed_gan_no/checkpoints/{cur_epochs}_%s_%s_os%d.pth' %
                  (opts.model, opts.dataset, opts.output_stride))
        save_ckpt('./fed_gan_no/checkpoints/latest_%s_%s_os%d.pth' %
                  (opts.model, opts.dataset, opts.output_stride))
        ...
        with open('./fed_gan_no/loss/epoch_loss_deeplab.txt', "a") as log_file:
            log_file.write('%s\n' % message)  # save the message

        epoch_loss = 0.0


        save_ckpt('/home/poac/7TB/yuanmingkang/deeplab_WHU2/deeplab_WHU2_train/fed_gan_no/checkpoints/%d_%s_%s_os%d.pth' %
